...
always @(posedge clock)  
  ...
  if (ready)
    case (state)
      READING:
        begin
         //speed <= { 1'b0, rd_data[3] & ~rd_data[2] };
         duplex <= rd_data[4];
         phy_connected <= rd_data[4] & rd_data[3] & ~rd_data[2];
         // duplex <= rd_data[14];
         //phy_connected <= rd_data[2];
         //duplex <= rd_data[14];
